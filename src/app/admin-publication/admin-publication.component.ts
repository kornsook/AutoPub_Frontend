...
export class AdminPublicationComponent implements OnInit {
  ...
  getPublications() {
    this.api.getCustomPublications().subscribe(data => {
  ...
  block(id: number, user_inx: number){
    this.active_publications = this.api.blockPublicationForUser(id, user_inx, this.active_publications);
    this.publications = [...this.active_publications]
  }

  remove(index: number){
    this.active_publications.splice(index, 1);
    this.publications = [...this.active_publications]
  }
